...
class EventManager
{
	...
	public function __construct() {
		$this->_logger = new LoggerCategory('Events');
	}
	...
	public function trigger(AbstractEvent $event) {
		...
		$this->_logger->log('Triggered event "'. $name .'" with '. count($this->_listeners[$name]) .' listeners.');

		foreach($this->_listeners[$name] as $i => $listener) {
			call_user_func_array($listener['callable'], array($event));

			if ($event->isPropagationStopped()) {
				$this->_logger->log('Stopped propagation of event "'. $name .'" at listener '. $i .'.');
				break;
			}
		}
	}
